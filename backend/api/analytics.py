...
async def download_csv_report(db: Session = Depends(get_db)):
    ...
    Fixed: previously returned 500 due to missing error handling and ORM attribute access issues.
    """
    try:
        output = io.StringIO()
        ...
        writer.writerow(["ID", "First Name", "Last Name", "Email", "Job Title",
                         "Lead Score", "Lead Status", "Lead Source", "Company ID", "Created At"])
        ...
        writer.writerow(["ID", "Name", "Stage", "Value ($)", "Probability (%)",
                         "Health Score", "Is Stalled", "Expected Close", "Created At"])
        ...
        writer.writerow(["ID", "Company ID", "Plan", "MRR ($)", "ARR ($)",
                         "Health Score", "Churn Risk", "Churn Probability (%)",
                         "Logins/Week", "CSAT Score", "NPS Score", "Created At"])
        ...
        for cu in customers:
            writer.writerow([
                ...
                cu.mrr or 0,
                cu.arr or 0,
                cu.health_score or 0,
                cu.churn_risk or "",
                cu.churn_probability or 0,
                cu.logins_per_week or 0,
                cu.csat_score or 0,
                cu.nps_score or 0,
                str(cu.created_at)[:19] if cu.created_at else "",
            ])
