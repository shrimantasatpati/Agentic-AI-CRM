...
const STEPS = [
  { name: 'Loading deal data', output: 'Deal record retrieved · Stage: Negotiation · Value: $125K' },
  { name: 'Calculating health score', output: 'Health score: 62/100 · Activity trend: declining' },
  { name: 'Predicting close probability', output: 'Close probability: 71% · Trend: ↓ from 84% last week' },
  { name: 'Checking for stall conditions', output: 'Last activity: 14 days ago · Threshold exceeded → STALLED' },
  { name: 'Identifying risk factors', output: '3 risk factors identified: timeline, champion, competition' },
  { name: 'Generating recommendations', output: '4 action items generated · Next: Executive outreach' },
  { name: 'Forecasting close date', output: 'Projected close: May 2, 2026 · Confidence: 68%' },
];
...
export default function PipelinePage() {
  ...
  const handleRun = useCallback(async (formData: Record<string, string>) => {
    ...
    try {
      await fetch(`http://localhost:8000/api/agents/analyze-deal/${formData.deal_id || 'deal_001'}`, {
        method: 'POST',
      }).catch(() => null);
    } catch { /* ignore */ }
    await new Promise((r) => setTimeout(r, STEPS.length * 500 + 600));
    setResult({ ...MOCK_RESULT, deal_name: formData.deal_name || MOCK_RESULT.deal_name });
    setIsComplete(true);
  }, []);
